...
| Column | Type | Description |
|--------|------|-------------|
| canonical_id | string | Borehole canonical ID (<bdi>FK → boreholes</bdi>.csv) |
...
2. Entry criteria: `n ≥ 4 AND has_detection AND n5 ≥ 1` — series not meeting <bdi>criteria → NONE</bdi>
...
Idempotency manifest written by `scripts/extract_zone_pdfs.py`. Tracks which PDFs have been processed (<bdi>PDF→text</bdi> via pdftotext) so re-runs skip already-extracted files. New PDFs added to `<Zone>/data/external/` or root `Base-Report/` (with `--include-shared`) trigger fresh extraction. Use `--force` to re-extract all.
...
| `decay_chains` | Per-borehole reductive dechlorination chains (<bdi>PCE→TCE</bdi>→DCE→VC, TCA→DCA) |
...
**TPFAS must never appear in any preset** (calculated <bdi>sum → double-counting</bdi>).
