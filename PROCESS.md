...
**עודכן אחרון**: 2026-06-10 (REQ #26 — Holon V7 PASS Gate 5)
...
|---|------|--------|--------|-------|-------|
| 28 | אורקסטרציה + גנריזציה לקראת 18 אזורים (פייפליין נקי מקצה-לקצה) | **שלב 1 הושלם**: (1) `generate_holon_data_pack.py` → `generate_zone_data_pack.py` — גנרי (`--zone`, נתיבים נגזרים), אומת **byte-identical** על חולון (7/7 CSVs) + Gate 2 PASS. (2) `scripts/run_pipeline.sh` — דרַייבר 7-שלבים שעוצר בגבולות Opus. (3) `ORCHESTRATION.md` — מפת מצה-לקצה (דטרמיניסטי/Opus, מוסכמת `--zone`, מצב גנריות). (4) Gate 5+6 — רשימת-מועמדים קשיחה (V6>V5) הוחלפה ב-glob גנרי-לגרסה `_latest_report()` → קולט V7/V8 אוטומטית (תיקן FAIL-שווא של V6-ברירת-מחדל). (5) 2 סקריפטי V4-era סומנו DEPRECATED. **נותר (שלב 2)**: (א) גנריזציית מחוללי-HTML (`generate_holon_v5_html`, `_designed`) — **חסום** ב-`report_designed/data_loader.py` שקורא בסיס-ראיות V4 מ-`lean_workspace/` עם שמות נעוצי-חולון (`severity_index_2025_holon.csv`); refactor גדול, **לא חוסם חולון**. (ב) איחוד מוסכמת `--zone` (אות-קטנה מול שם-ספרייה) ב-`cli_common.py`+סקריפטי-ליבה. (ג) rename `embed_holon_figures_v5`. (ד) ריצת אזור-2 מקצה-לקצה לאימות גנריות. | ⚠️ Partial (שלב 1 done) | 2026-06-10 | scripts/generate_zone_data_pack.py, run_pipeline.sh, qa_pipeline.py, ORCHESTRATION.md, toolkit/playbooks/* |
| 27 | הערות הידרולוג סבב-2 על V7 — איורים + תוכן | **Part A (איורים, דטרמיניסטי) הושלם ב-1588ba7**: (1) `wrap_bidi(svg)` הזריק `<bdi>` (תג HTML לא-חוקי) לתוך SVG `<text>` → הסתיר שנים/תוויות-מזהם/DWS; הוסר — ה-SVG מוטמע raw. (2) `build_figures()` נתיב קשיח ל-V5 → משתמש בדוח המועבר. (3) `_time_series_panel` ציר-X לפי שנה מעוגלת → שתי דגימות באותה שנה נערמו לקו אנכי שנראה כמחבר שני מזהמים; שונה ל-fractional-year. אימות: רינדור PNG מאשר שנים+תוויות+DWS; Gate 5/6 PASS. **Part B הושלם ב-563c98b**: (4) עמודת "תאריך דיגום אחרון" בטבלאות §3. (5) הרחבת §2 — הידרוגיאולוגיה (ללא DNAPL) + פסקת אפיון גיאוגרפי/כרונולוגי/אופי-תעשיות. (6) שאלת אבחון על קידוחי הפקת מי-שתייה + ניתוח פורנזי בסיסי (אילו קידוחי ניטור/תעשייה משפיעים). תבניות updated + zone_diagnosis_prompt.md re-rendered. **Part C נותר (Opus 4+5)**: (7) ריצת Opus Step 4 → NEW zone_diagnosis.md (עם production wells + focus order). (8) render zone_report_prompt.md Step 5 עם NEW diagnosis. (9) ריצת Opus Step 5 → HOLON_REPORT_V8.md (עם §2 הידרוגיאולוגיה/גיאוגרפיה + דוח ניקוי טבלות תאריך). (10) generate V8 HTML + QA gates. (11) דה-דופ `אגד_איזור_7`→`אזור_7` (אופציונלי — נדחה תלוי בבדיקה דטרמיניסטית); (12) סנכרון ל-holon-v6. | ⚠️ Partial (A+B done, C blocked on Opus) | 2026-06-10 | scripts/templates/zone_diagnosis_prompt_template.md, zone_report_prompt_template_v5.md; Holon/context_pack/05_prompt/zone_diagnosis_prompt.md |
