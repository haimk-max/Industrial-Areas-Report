...
**עודכן אחרון**: 2026-05-26 (REQ #16 — Exec-Summary Engine organized to report-engine/; REQ #15 — Executive Summary HTML + YAML Design Spec; REQ #13.6 — V5 report final)
...
|---|------|--------|--------|----------------|--------|
| 17 | רכיב גנרי: יצירת brief YAML מהדוח המלא (Full Report → Engine Brief) | **החוליה החסרה** בין הדוח המלא (V5.md) למנוע התקצירים (report-engine). עיקרון: **הדוח המלא הוא SSOT לכל התוכן**; רק קואורדינטות מדויקות מגיעות דטרמיניסטית מ-zone_wells.csv (ITM→WGS84), כי הדוח לא נושא מיקומי-מפה ו-coords שניחש Opus נמצאו שגויים ~1km. **נבנה**: (1) `scripts/templates/zone_brief_prompt_template.md` — פרומפט Opus גנרי (מבנה Anthropic XML, placeholders {ZONE_ID}/{FULL_REPORT}/{SCHEMA}/{VOICE_RULES}/{WELL_INVENTORY}); dual-framing internal/public, ייחוס גנרי, marker `# COORDS-INJECT:<name>`. (2) `scripts/generate_zone_brief.py` — `prepare` (zone_wells→ITM→WGS84 lookup + הרכבת פרומפט) + `finalize` (הזרקת coords **טקסטואלית** ששומרת פורמט/הערות + ולידציה מבנית). **נבדק end-to-end** (2026-05-26): prepare (112 קידוחים) → Opus מפיק brief מלא מ-V5.md (7 findings, 11 boreholes, 4 sources, 4 decision-cats) → finalize: validation נקי, 10/11 coords + 4 sources הוזרקו, **מק חולון 14 תוקן ל-[32.01068,34.7899]** (מול [32.01510,34.78090] השגוי של Claude Design), 0 דליפות שמות ל-public, פורמט/הערות נשמרו. **נותר** (follow-ups קטנים): (a) הרצת המנוע (report-engine) על ה-brief המיוצר → INTERNAL/PUBLIC.html (הוכחה סופית brief→HTML); (b) edge-case: אשכולות/מתקנים שאינם קידוח ב-zone_wells (רימטל → warning, coords [0,0]); (c) עדכון ZONE_REPORT_PROCESS_GUIDE עם הצעד (step 5.5: report→brief). | ⚠️ Partial | 2026-05-26 | scripts/generate_zone_brief.py, scripts/templates/zone_brief_prompt_template.md |
...
|---|------|---------------|--------|----------|
| 16 | ארגון מנוע התקצירים הגנרי (Exec-Summary Engine) | 2026-05-26 | (בתהליך) | **ENGINE ORGANIZED**: המשתמש ייצר ב-Claude Design מנוע גנרי לתקצירי מנהלים (zone-agnostic). הקבצים הועלו שטוחים ל-Holon/output/ עם בלבול שמות; תוקן בהעלאה חוזרת. **סודרו ל-`report-engine/` בשורש** (גנרי — לא תחת Holon/): (1) `CLAUDE.md` — operating manual לסוכן הבנייה (field-mappings, quality-bar, anti-patterns). (2) `design-system/` FROZEN: tokens.css (SSOT ל-CSS vars), architecture.md (twins philosophy), typography.md (RTL+fonts), color.md (semantic palette), voice.md (internal/public tone), README.md + reference/ (HOLON_INTERNAL/PUBLIC/index.html — gold-standard). (3) `schemas/zone-brief.schema.json` (316ש' JSON Schema, ולידציה). (4) `briefs/holon.yaml` (610ש' — כל נתוני חולון; placeholder ~58 קידוחי ליבה). (5) `output/holon/{INTERNAL,PUBLIC}.html` (הפלט; היה כפילות byte-identical ל-reference — פוצל reference מול output). 14 קבצים הועברו ב-git mv (היסטוריה נשמרה) + README.md ראשי נוצר. עיקרון: per-zone data רק ב-briefs/<zone>.yaml; design-system+schema קבועים לכל 18 האזורים. |
