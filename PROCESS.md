...
**עודכן אחרון**: 2026-06-10 (REQ #28 both stages complete; REQ #27 closed Holon V8)
...
|---|------|--------|--------|-------|-------|
| 29 | שלמות לקראת אריזה — דוחות ניהוליים + RAG + סנכרון מעקב | אודיט עומק (2 סוכנים) חשף 3 פערים + 3 רגרסיות מ-REQ#28. **כל השלבים ✅ (5e0044d, d5ef97e)**. **שלב 3 — קבצי מעקב**: HANDOVER/LESSONS/REQUIREMENTS סונכרנו ל-V8; WORD_REVIEW נכתב מחדש (נתיבי rerun_v2 מחוקים→Holon/output, LibreOffice-broken מתועד); MERGE_PLAN+SESSION_SUMMARY→docs/legacy/ עם הערות-סגירה. **שלב 2 — RAG**: דחייה רשמית — banner DRAFT ב-SKILL.md, LESSONS §3.3a, PROCESS #14 + REQUIREMENTS. **שלב 1 — תשתית exec-summary**: de-hardcode V5→`_latest_report_md()` glob; provenance (source_report version+sha); `run_pipeline.sh` stage `exec-summary` (Step 8) + staleness check; ORCHESTRATION+report-engine README; באנר "מיושן" על 2 ה-exec summaries; תיקון קישורים שבורים ב-reference/index.html. **רגרסיות REQ#28 שתוקנו**: (1) **שחזור Holon/02_data/** (7 CSVs — ה-Data Pack הקנוני, נמחק בטעות; Gate 2 PASS); (2) תיקון casing (holon→Holon dir) ב-2 מחוללים+data_loader; (3) תיקון masthead שבור (NameError) → מטא-דאטה דינמי נכון (112 קידוחים, 15,173 מדידות). **+ תיקון בדיקה שבורה**: test_param_families (is_btex→is_fuel; BTEX→FUEL taxonomy) — 28→38 passing. **נדחה**: תוכן V8 של הדוחות הניהוליים עד אישור הידרולוג; quirk סיווג anions→METALS (לבדיקה). | ✅ Closed | 2026-06-10 | HANDOVER, LESSONS, REQUIREMENTS, WORD_REVIEW, docs/legacy/*, agent-rag/SKILL.md, generate_zone_brief.py, run_pipeline.sh, ORCHESTRATION.md, report-engine/*, generate_holon_v5_html.py, generate_holon_designed.py, data_loader.py, Holon/02_data/*, tests/test_param_families.py |
| 28 | אורקסטרציה + גנריזציה לקראת 18 אזורים (פייפליין נקי מקצה-לקצה) | **BOTH STAGES COMPLETE (ba51451, 2026-06-10)**. **Stage 1**: data_pack genericized (`--zone` flag, byte-identical); Gate 5+6 version-generic glob (`_latest_report()` catches V7/V8+); ORCHESTRATION.md documented. **Stage 2**: HTML generators fully genericized: (1) `data_loader.py` refactored (zone parameter on all load_* functions, dynamic workspace path); (2) `generate_holon_v5_html.py` + `generate_holon_designed.py` accept `--zone` flag + auto-detect paths; (3) Legacy scripts (7 V4-era) archived to `scripts/legacy/` with README + migration paths; (4) Holon/02_data/ + rerun_v2_* experiment dirs deleted (superseded by lean_workspace); (5) QA Gates hardened (DOCX fallback fixed, diagnosis_sha TODO noted, focus-order cross-check flag added); (6) Documentation synced (CLAUDE.md, PROCESS_GUIDE, PROCESS.md, toolkit/playbooks). **Result**: Full 7-step pipeline (data pack → Opus → HTML) now zone-generic. Holon V8 artifacts verified intact. Ready for 18-zone activation. | ✅ Closed | 2026-06-10 | scripts/generate_zone_data_pack.py, run_pipeline.sh, qa_pipeline.py, generate_holon_v5_html.py, generate_holon_designed.py, data_loader.py, scripts/legacy/, CLAUDE.md, PROCESS_GUIDE.md |
| 27 | הערות הידרולוג סבב-2 על V7 — איורים + תוכן | **Part A+B+C הושלמו → HOLON_REPORT_V8.md/.html**. Part A (1588ba7): bdi-in-SVG, נתיב-קשיח, ציר fractional-year. Part B (563c98b): עמודת תאריך-דיגום §3, הרחבת §2 הידרו/גיאו, שאלת קידוחי-הפקה. **Part C (Opus 4+5)**: Step 4 (Opus #1) → אבחון חדש 7 מוקדים **+ ניתוח קידוחי הפקה** (מוקד #6 "ציר קידוחי ההפקה") — Gate 4 PASS; תיקון דריפט תבנית (`anchors_pilot.yaml` → 2 קבצים אמיתיים); רינדור-מחדש prompt מהאבחון החדש (חוזה staleness); Step 5 (Opus #2) → **HOLON_REPORT_V8.md** (8 סעיפים, סדר-מוקדים תואם, §2 מורחב, עמודת תאריך) → 2 תיקוני-פרוזה ('V5 Hybrid Pipeline', 'כמעט מוחלט') → **Gate 5 PASS (0/0)**; V8 HTML (225KB) → **Gate 6 PASS**. **נדחה**: (11) דה-דופ `אגד_איזור_7` (לא חוסם); (12) סנכרון ל-holon-v6. | ✅ Closed (V8 + כל השערים) | 2026-06-10 | Holon/output/HOLON_REPORT_V8.md/.html, zone_diagnosis.md, templates |
