...
**עודכן אחרון**: 2026-06-14 (REQ #31 closed — Deep Infrastructure Fixes: generator full-section coverage, Gate 8 brief/anonymization, Step 8 docs, diagnosis_sha footgun; #27-30 audit-trail)
...
|---|------|---------------|--------|----------|
| 31 | Deep Infrastructure Fixes — generator coverage, brief/anonymization gates, pipeline docs, staleness footgun | 2026-06-14 | (this commit) | **COMPLETE — 5 sub-tasks**. **31.1** (חוסם אזור שני — נפתר): `generate_zone_html_from_brief.py` שוכתב עם `replace_container_inner()` (depth-counting של divs מקוננים) — כל סעיפי ה-brief נצרכים כעת כולל `stats_public`/`timeline`/`means_summary`/`methodology` (לפני: רק 5, השאר hardcoded-חולון). אימות: ספירות .stat=3/.event=7/.means-card=4/.method-cell=3/findings=9 = len(brief); 13 טסטים חדשים; design-system קפוא לא נגע. **31.2**: `qa_pipeline.py` **Gate 8** (חדש, post-approval) — brief↔report sha: FAIL אם `source_report_sha256_12` לא תואם דוח אחרון. holon.yaml קיבל חותמת provenance machine-readable (V8/9d789b15c55c). **31.3**: Gate 8 אנונימיזציה — שמות-מתקנים (sources[].name_internal) ב-PUBLIC=FAIL; שמות-קידוחים=WARN. אימות: PUBLIC נקי מ-7 שמות-מתקנים. **31.4**: PROCESS_GUIDE §VIII Step 8 (<bdi>Brief→Exec</bdi> Summaries) + ניקוי חוב governance (REQ #27-30 הועברו ל-Closed; ORCHESTRATION/CLAUDE חותמים עודכנו). **31.5** (footgun): `render_zone_prompt.py` חותם `diagnosis_sha256_12` (report step); Gate 3 מאמת מול zone_diagnosis.md חי. אימות: Gate 3 PASS (diagnosis_sha תואם), Gate 8 WARN(0 errors), כל השערים 2-6 ללא שגיאות, 51 טסטים. נדחה (out-of-scope, schema): byte-exact reference reproduction (bdi-per-token + spark bars לא ב-brief schema). |
| 30 | V8 exec-summaries — brief מ-V5-era ל-V8 + INTERNAL/PUBLIC HTML | 2026-06-14 | ce6436c | **COMPLETE** — brief עודכן ל-V8 (9 ממצאים כולל 3 מוקדי דלק, 16 קידוחים, 7 מקורות); INTERNAL+PUBLIC HTMLs נוצרו (65KB+52KB); דליפת "רימטל" ב-PUBLIC reference תוקנה ל-"אתר מחזור מתכות משוקם"; מגבלת ה-generator (5/9 סעיפים) תועדה ב-<bdi>HANDOVER → REQ</bdi> #31.1. |
| 29 | שלמות לקראת אריזה — דוחות ניהוליים + RAG + סנכרון מעקב | 2026-06-10 | 5e0044d, d5ef97e | אודיט עומק (2 סוכנים) חשף 3 פערים + 3 רגרסיות מ-REQ#28. כל השלבים ✅. קבצים: HANDOVER, LESSONS, REQUIREMENTS, WORD_REVIEW, docs/legacy/*, agent-rag/SKILL.md, generate_zone_brief.py, run_pipeline.sh, ORCHESTRATION.md, report-engine/*, generate_holon_v5_html.py, generate_holon_designed.py, data_loader.py, Holon/02_data/*, tests/test_param_families.py. |
| 28 | אורקסטרציה + גנריזציה לקראת 18 אזורים (פייפליין נקי מקצה-לקצה) | 2026-06-10 | ba51451 | **BOTH STAGES COMPLETE**. Stage 1: data_pack genericized (`--zone`, byte-identical); Gate 5+6 version-generic glob (`_latest_report()` catches V7/V8+); ORCHESTRATION.md documented. Stage 2: HTML generators genericized (data_loader.py zone param; generate_holon_v5_html.py + generate_holon_designed.py `--zone` + auto-detect; 7 V4-era scripts archived to scripts/legacy/; QA Gates hardened — diagnosis_sha TODO <bdi>noted → REQ</bdi> #31.5; docs synced). Full 7-step pipeline zone-generic. Holon V8 artifacts intact. |
| 27 | הערות הידרולוג סבב-2 על V7 — איורים + תוכן  — HOLON_REPORT_V8 | 2026-06-10 | 1588ba7, 563c98b, (V8) | **Part A+B+<bdi>C → HOLON_REPORT_V8</bdi>.md/.html**. Part A: bdi-in-SVG, נתיב-קשיח, ציר fractional-year. Part B: עמודת תאריך-דיגום §3, הרחבת §2 הידרו/גיאו, שאלת קידוחי-הפקה. Part C (Opus 4+5): אבחון חדש 7 מוקדים + ניתוח קידוחי-הפקה (Gate 4 PASS); רינדור-מחדש prompt (חוזה staleness); HOLON_REPORT_V8.md (8 סעיפים) → Gate 5 PASS (0/0); V8 HTML 225<bdi>KB → Gate</bdi> 6 PASS. נדחה: דה-דופ `אגד_איזור_7`, סנכרון holon-v6. |
| 26 | הערות הידרולוג על V6  — תשתית טרמינולוגיה+נימה+איורים+QA נתונים | 2026-06-10 | a607cf7 (תשתית), V7-commit | **COMPLETE** — (1) STYLE_GUIDE §B.5 SSOT: ריקבון —פירוק, שתיקה —הפסקת ניטור, HIGH/MEDIUM/LOW —רמת ודאות גבוהה/בינונית/נמוכה. (2) `render_zone_prompt.py` מזריק `{TERMINOLOGY_BLOCK}` מ-§B.5 לשני שלבי Opus (Gate 3 בודק staleness). (3) Gate 2 coherence: מזהים-כפולים + סדרות-מנותקות (WARN). (4) svg_charts: direction:ltr, DWS-param label, Year axis, on-curve labels, ביטול re-sort/cap. (5) **HOLON_REPORT_V7.md** — Opus Step 5 עם הפרומפט החדש  — **Gate 5 PASS (0 שגיאות, 0 אזהרות)**. כל המינוחים האסורים נעלמו. |
...
| 18 | ENGINE: <bdi>Brief → Twin</bdi> HTML generator | 2026-05-26 | 357751c | **COMPLETE**: `scripts/generate_zone_html_from_brief.py` reads brief YAML + clones frozen reference templates + data-replaces per field-mappings (CLAUDE.md §5) → two self-contained HTML files. Holon verified: 64KB INTERNAL (real names, decision matrix) + 52KB PUBLIC (generic only, no decisions). All sections injected: KPI grid, family ledger, 7 findings w/ dual framing, decisions table, metadata. Design system preserved (CSS vars, fonts, RTL, Leaflet). **Final proof**: entire pipeline V5.<bdi>md → brief</bdi>.yaml → HTML operational. Zone-agnostic. |
| 17 | ENGINE: Full <bdi>Report → Brief</bdi> YAML generator | 2026-05-26 | (multiple) | **COMPLETE**: `scripts/generate_zone_brief.py` (two-stage: prepare + finalize) + zone_brief_prompt_template.md (Opus input). Holon verified: prepare assembles 112-well <bdi>ITM→WGS84</bdi> lookup + prompt; Opus produces 7 findings w/ dual framing + 11 boreholes + 4 sources + 4 decision-cats; finalize validates schema + injects coords textually (preserves format/comments) + flags leaks. **Critical fix**: מק חולון 14 coords corrected to [32.01068, 34.7899] (vs. Claude Design error). Edge case: רימטל facility not in <bdi>wells → warning</bdi> + [0,0]. |
...
- ✅ PR #19 פתוחה (feature/<bdi>hybrid-v5-implementation → main</bdi>)
