...
|---|------|---------------|--------|----------|
| 34 | REQ #34 — תיקון שלמות נתונים (בעיית 46+63=111) + DOCS_MAP.md | 2026-06-16 | 18a6acf, a060119 | **COMPLETE**. (1) **שגיאת בעיית נתונים**: דוח Holon V8 הצהיר "46 ניטור כללי + 63 דלק = 111" אך השגיאה הייתה בשני הנתונים (46+63=109, לא 111). אמת-קרקע מ-selected_boreholes.json: 40 תעשייתי + 64 דלק + 6 ניטור + 1 פרטי = 111. הפילוח הנכון: 47 לא-דלק + 64 דלק = 111. תוקן ב-6 קבצים: HOLON_REPORT_V8.md (3 מקומות), briefs/holon.yaml (3 מקומות + sha256), README.md (תווית raw/scoped), HTML (regenerate). (2) **DOCS_MAP.md**: קובץ חדש שמסווג כל governance/doc ל-3 מחזורי-חיים (חיים/SSOT/היסטוריה) עבור קוראים חדשים. (3) **אימות**: 0 מופעים של 46/63; Gate 2 PASS (111 קידוחים עקביים); Gate 8 PASS (brief↔report sha, PUBLIC נקי); HANDOVER עודכן עם 3 footguns חדשים. |
| 33 | איחוד ענפים — Raanana V5 pipeline merge + branch consolidation + toolkit move + qa Gate 6 zone-relative | 2026-06-16 | d69d9e4, 0341772 | **COMPLETE**. (1) **העברת toolkit**: `docs/GENERIC_PDF_EXTRACTION_AND_RETRIEVAL.md` → `toolkit/playbooks/generic_pdf_extraction_and_retrieval.md`; 2 הפניות ב-PDF_QA_INFRASTRUCTURE.md + 1 ב-.html עודכנו. (2) **מיזוג `claude/raanana-v5-pipeline`** לענף הראשי (42 קבצים, +9,580 שורות) — כל פייפליין V5 של רעננה נכנס (01_scope, 02_data 6 CSVs, context_pack, 04_diagnosis, חילוץ PDF, RAANANA_REPORT_V5.md/.html, build_zone_lean_workspace.py). רעננה היא כעת האזור ה-2 שעבר V5 end-to-end (אחרי חולון). (3) **qa_pipeline.py Gate 6**: סף ספירת עיגולי-מפה הפך יחסי-לאזור (`_count_scoped_wells` × 0.8, מינ' 3) במקום ≥50 קבוע — קריטי לאזורים קטנים (רעננה 7 קידוחים). מוזג אוטומטית עם Gate 8 (REQ #31.3). (4) **הכרעות קונפליקט**: CLAUDE/HANDOVER/2 exec-summaries → גרסת ראשי; qa_pipeline.py → auto-merge (שני השערים). (5) **ניקוי**: ענף raanana מקומי + worktree נמחקו; remote נותר stale (git proxy דוחה מחיקה — מחיקה ידנית ב-UI). אימות: py_compile עובר; אין conflict markers; CLAUDE.md שמר Doc-Sync; Raanana V5 files נוכחים. |
| 32 | יישור כיתובי איורים (5-fig) + RTL fix מצומצם לדוחות | 2026-06-16 | 233f2d0, 168218a | **COMPLETE**. (1) **כיתובי איורים**: תבנית V5 prompt עודכנה ל-5 איורים (לא 6: מפה, CVOC, METALS, FUEL, gaps); כיתובי איור 2–5 ב-HOLON_REPORT_V8.md שוכתבו להתאמה לתוצר המרונדר; V8.html נוצר מחדש עם הזרקת איורים. **שורש**: prompt הורה על 6 אך ה-generator מזריק 5. (2) **fix_rtl_arrows.py** (כלי חדש, reusable): תיקון חצים ל-`<bdi>`/em-dash. הוחל **רק על דוחות deliverable** (Holon V4–V8 + exec summaries, Raanana reports + drilling cards, report-engine outputs) — לאחר הערת המשתמש צומצם מ-88 קבצים (commit 3bbfd9f) חזרה ל-deliverables בלבד (commit 168218a), כי em-dash פוגע בבהירות בקבצי spec ("ND → NULL"). אימות: V8.md 60 em-dashes; קבצי תהליך חזרו ל-233f2d0; עץ נקי. |
| 31 | Deep Infrastructure Fixes — generator coverage, brief/anonymization gates, pipeline docs, staleness footgun | 2026-06-14 | (this commit) | **COMPLETE — 5 sub-tasks**. **31.1** (חוסם אזור שני — נפתר): `generate_zone_html_from_brief.py` שוכתב עם `replace_container_inner()` (depth-counting של divs מקוננים) — כל סעיפי ה-brief נצרכים כעת כולל `stats_public`/`timeline`/`means_summary`/`methodology` (לפני: רק 5, השאר hardcoded-חולון). אימות: ספירות .stat=3/.event=7/.means-card=4/.method-cell=3/findings=9 = len(brief); 13 טסטים חדשים; design-system קפוא לא נגע. **31.2**: `qa_pipeline.py` **Gate 8** (חדש, post-approval) — brief↔report sha: FAIL אם `source_report_sha256_12` לא תואם דוח אחרון. holon.yaml קיבל חותמת provenance machine-readable (V8/9d789b15c55c). **31.3**: Gate 8 אנונימיזציה — שמות-מתקנים (sources[].name_internal) ב-PUBLIC=FAIL; שמות-קידוחים=WARN. אימות: PUBLIC נקי מ-7 שמות-מתקנים. **31.4**: PROCESS_GUIDE §VIII Step 8 (Brief→Exec Summaries) + ניקוי חוב governance (REQ #27-30 הועברו ל-Closed; ORCHESTRATION/CLAUDE חותמים עודכנו). **31.5** (footgun): `render_zone_prompt.py` חותם `diagnosis_sha256_12` (report step); Gate 3 מאמת מול zone_diagnosis.md חי. אימות: Gate 3 PASS (diagnosis_sha תואם), Gate 8 WARN(0 errors), כל השערים 2-6 ללא שגיאות, 51 טסטים. נדחה (out-of-scope, schema): byte-exact reference reproduction (bdi-per-token + spark bars לא ב-brief schema). |
| 30 | V8 exec-summaries — brief מ-V5-era ל-V8 + INTERNAL/PUBLIC HTML | 2026-06-14 | ce6436c | **COMPLETE** — brief עודכן ל-V8 (9 ממצאים כולל 3 מוקדי דלק, 16 קידוחים, 7 מקורות); INTERNAL+PUBLIC HTMLs נוצרו (65KB+52KB); דליפת "רימטל" ב-PUBLIC reference תוקנה ל-"אתר מחזור מתכות משוקם"; מגבלת ה-generator (5/9 סעיפים) תועדה ב-HANDOVER → REQ #31.1. |
