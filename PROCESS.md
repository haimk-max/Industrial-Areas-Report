...
|---|------|--------|--------|-------|-------|
| 27 | הערות הידרולוג סבב-2 על V7 — איורים + תוכן | **Part A (איורים, דטרמיניסטי) הושלם ב-1588ba7**: (1) `wrap_bidi(svg)` הזריק `<bdi>` (תג HTML לא-חוקי) לתוך SVG `<text>` → הסתיר שנים/תוויות-מזהם/DWS; הוסר — ה-SVG מוטמע raw. (2) `build_figures()` נתיב קשיח ל-V5 → משתמש בדוח המועבר. (3) `_time_series_panel` ציר-X לפי שנה מעוגלת → שתי דגימות באותה שנה נערמו לקו אנכי שנראה כמחבר שני מזהמים; שונה ל-fractional-year. אימות: רינדור PNG מאשר שנים+תוויות+DWS; Gate 5/6 PASS. **Part B/C נותר (פרומפט + Opus V8)**: (4) עמודת "תאריך דיגום אחרון" בטבלאות §3; (5) הרחבת §2 — הידרוגיאולוגיה (ללא DNAPL) + פסקת אפיון גיאוגרפי/כרונולוגי/אופי-תעשיות; (6) שאלת אבחון על קידוחי הפקת מי-שתייה + ניתוח פורנזי בסיסי (אילו קידוחי ניטור/תעשייה משפיעים); (7) דה-דופ `אגד_איזור_7`→`אזור_7` (מקור→trend_analysis→pack regen); (8) ריצת Opus 4+5 → V8; (9) סנכרון ל-holon-v6. | ⚠️ Partial (Part A done) | 2026-06-10 | scripts/generate_holon_v5_html.py, svg_charts.py, templates/*, generate_holon_data_pack.py |
