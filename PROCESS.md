...
|---|------|--------|--------|----------------|--------|
| 17 | רכיב גנרי: יצירת brief YAML מהדוח המלא (Full Report → Engine Brief) | **החוליה החסרה** בין הדוח המלא (V5.md) למנוע התקצירים (report-engine). עיקרון: **הדוח המלא הוא SSOT לכל התוכן**; רק קואורדינטות מדויקות מגיעות דטרמיניסטית מ-zone_wells.csv (ITM→WGS84), כי הדוח לא נושא מיקומי-מפה ו-coords שניחש Opus נמצאו שגויים ~1km. **נבנה**: (1) `scripts/templates/zone_brief_prompt_template.md` — פרומפט Opus גנרי (מבנה Anthropic XML, placeholders {ZONE_ID}/{FULL_REPORT}/{SCHEMA}/{VOICE_RULES}/{WELL_INVENTORY}); dual-framing internal/public, ייחוס גנרי, marker `# COORDS-INJECT:<name>`. (2) `scripts/generate_zone_brief.py` — `prepare` (zone_wells→ITM→WGS84 lookup + הרכבת פרומפט) + `finalize` (הזרקת coords **טקסטואלית** ששומרת פורמט/הערות + ולידציה מבנית). **נבדק**: prepare (112 קידוחים), textual injection (מק חולון 14→[32.01068,34.7899] מדויק; פורמט נשמר), validation. **נותר**: (a) הרצת end-to-end על חולון (Opus מפיק brief מלא → השוואה ל-reference holon.yaml); (b) name-matching לאשכולות/מתקנים שלא ב-zone_wells (אשכול אגד, רימטל — warning); (c) עדכון ZONE_REPORT_PROCESS_GUIDE עם הצעד החדש (step 5.5: report→brief). | ⚠️ Partial | 2026-05-26 | scripts/generate_zone_brief.py, scripts/templates/zone_brief_prompt_template.md |
