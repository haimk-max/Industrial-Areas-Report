...
|---|------|--------|--------|-------|-------|
| 29 | שלמות לקראת אריזה — דוחות ניהוליים + RAG + סנכרון מעקב | אודיט עומק (2 סוכנים) חשף 3 פערים. **(A) דוחות ניהוליים (INTERNAL+PUBLIC)**: מיושנים (V5-era 26/5) מול דוח V8; `generate_zone_brief.py:72` נעוץ ל-`_REPORT_V5.md`; שני הסקריפטים יתומים מ-`run_pipeline.sh`/`ORCHESTRATION.md`; אין staleness contract; `briefs/holon.yaml` חסר version provenance; עוגנים נעוצי-חולון. **(B) RAG**: `agent-rag` skeleton-only, אפס קוד, יתום מקבצי-מעקב. **(C) קבצי מעקב**: REQUIREMENTS/SESSION_SUMMARY/MERGE_PLAN/WORD_REVIEW מיושנים (V5, נתיבים מחוקים). **החלטות משתמש**: תשתית-עכשיו/תוכן-אחרי-אישור; RAG דחייה רשמית. **שלב 3 (✅ done)**: HANDOVER/LESSONS/REQUIREMENTS סונכרנו ל-V8; WORD_REVIEW נכתב מחדש (V8/Holon/output, LibreOffice-broken מתועד); MERGE_PLAN+SESSION_SUMMARY→docs/legacy/ עם הערות-סגירה; RAG banner + LESSONS §3.3a. **שלב 1 (בתהליך)**: תשתית exec-summary (de-hardcode, חיווט, staleness, provenance, באנר-stale). **תוכן V8 נדחה עד אישור הידרולוג.** | 🔄 In Progress | 2026-06-10 | HANDOVER.md, LESSONS.md, REQUIREMENTS.md, WORD_REVIEW_PROCESS.md, docs/legacy/*, agent-rag/SKILL.md, generate_zone_brief.py, run_pipeline.sh, ORCHESTRATION.md |
