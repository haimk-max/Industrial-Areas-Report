...
**עודכן אחרון**: 2026-06-14 (REQ #30 closed + audit-trail; #27/#28/#29 הועברו ל-Closed; REQ #31 פתוח — Deep Infrastructure Fixes)
...
|---|------|--------|--------|-------|-------|
| 31 | Deep Infrastructure Fixes — generator coverage, brief/anonymization gates, pipeline docs, staleness footgun | חמישה תיקונים עמוקים שזוהו בעבודת V8 exec-summaries (HANDOVER 2026-06-14). **31.1** (חוסם אזור שני): `generate_zone_html_from_brief.py` מחליף רק 5 סעיפים (context_intro, findings, framing_warning, zone_name, doc_id); סעיפי stats_public/means_summary/methodology/timeline נשארים hardcoded מה-reference → להוסיף replacement logic לכל הסעיפים. **31.2**: Gate חדש brief↔report sha — FAIL אם `source_report_sha` ב-brief לא תואם את דוח-הקובץ הנוכחי (מונע exec-summaries מיושנות). **31.3**: Gate חדש אכיפת-אנונימיזציה — סריקת PUBLIC HTML מול רשימת שמות-מתקנים; FAIL על דליפה (מונע חזרה של דליפת "רימטל"). **31.4**: תיעוד פייפליין — "Step 8: Brief → Exec Summaries" ל-PROCESS_GUIDE §VIII + brief כ-standard artifact ב-§I; **+ ניקוי חוב governance קיים** (PROCESS.md #30 רשום ב-header בלבד; ORCHESTRATION/CLAUDE חותם מיושן). **31.5** (footgun): חתימת `diagnosis_sha` בפרומפט (בנוסף ל-template_sha256), Gate 3 מאמת התאמה (מונע focus_order מיושן אם Step 4 רץ מחדש). | 🔄 In Progress | 2026-06-14 | generate_zone_html_from_brief.py, qa_pipeline.py, render_zone_prompt.py, ZONE_REPORT_PROCESS_GUIDE.md, ORCHESTRATION.md, CLAUDE.md |
