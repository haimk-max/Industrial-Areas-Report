...
|---|------|--------|--------|-------|-------|
| 26 | הערות הידרולוג על V6 → תשתית (טרמינולוגיה+נימה+איורים+QA נתונים) | **תשתית הושלמה ואומתה (לא ריצת Opus עדיין)**. שורש: STYLE_GUIDE + validate_report קיימים אך מנותקים ממסלול V5/V6 (אותו drift כמו REQ #25). בוצע: (1) **טרמינולוגיה SSOT** — STYLE_GUIDE §A.3/§B.5: "פירוק" (לא ריקבון), "הפסקת ניטור" (לא שתיקה), "רמת ודאות גבוהה/בינונית/נמוכה" (לא HIGH/MEDIUM/LOW); §H.3 הבהרה prose-vs-JSON; זורם לפרומפט דרך `render_zone_prompt.py` (`{TERMINOLOGY_BLOCK}` מ-§B.5). (2) **נתונים** — Gate 2 coherence (מזהים-כפולים + סדרות-מנותקות; WARN). (3) **פרומפט** — Step 4/5: מילון מוזרק + כותרות §3 גיאוגרפיות (HARD) + טבלת "ריכוז אחרון (מקס' היסטורי)" + §5 רמת-ודאות עברית + הסבר אופן-דירוג + תוקנה סתירת "HIGH/MEDIUM/LOW מותר". (4) **איורים** — svg_charts: כותרת-ציר Year + DWS-עם-פרמטר (TCE/Cr/Benzene) + תווית-מזהם על-עקום + שנים/Y כהים + root direction:ltr + **כיבוד בחירת-Opus** (ביטול re-sort/cap ב-override). שערים: 5 (לא 7), 3 משודרגים, 0 חדשים. **אימות**: Gate 3 PASS (re-render); Gate 2 WARN — **תפס נד_אגד_אזור_7/איזור_7** + 5 סדרות; Gate 5 FAIL על V6 הקיים (ריקבון×6/קיצוני×10/HIGH×12 — צפוי, קדם-לכלל); svg smoke-test PASS. **נותר**: ריצת Opus (Step 4→5) ליצירת V7 נקי + רינדור איורים. | ⚠️ Partial (תשתית הושלמה; Opus pending) | 2026-06-09 | docs/STYLE_GUIDE.md, scripts/templates/*, svg_charts.py, qa_pipeline.py, render_zone_prompt.py |
