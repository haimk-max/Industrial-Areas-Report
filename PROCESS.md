...
|---|------|--------|--------|-------|-------|
| 25 | Prompt-layer SSOT drift — Gate 3 + render enforcement | **שורש**: REQ #24 עשה את §IV ל-SSOT והפך מסמכי-ניהול להפניות, אבל **שכבת ה-prompt (templates + rendered prompts + playbooks) — שמניעה בפועל את היצירה — לא נכללה**, ונסחפה. אודיט מצא 6 נקודות-כשל: (1) rendered report prompt family-first; (2) כפילות מיושנת ב-lean_workspace/05_prompt; (3) template V4 footgun; (4) template אבחון ללא הוראת `## סדר מוקדים` + שמות-קלט ישנים; (5) playbook שמות-קלט ישנים; (6) דוגמאות-ציטוט שיוריות. **תיקון Phase 1 (done)**: `render_zone_prompt.py` (רינדור דטרמיניסטי לשני שלבי Opus + provenance stamp); `qa_pipeline.py` **Gate 3 (Prompt Layer)** — תופס שפת-סדר מבוטלת/placeholders/drift-מול-template/כפילויות; תיקון templates #3/#4 + playbook #5; באנר DEPRECATED ל-V4. **Phase 2 (pending)**: ריצה נאמנה — Opus שלב 4+5 דרך ה-prompts הקנוניים המרונדרים. | 🔧 doing | 2026-06-08 | scripts/render_zone_prompt.py, scripts/qa_pipeline.py, scripts/templates/, toolkit/playbooks/ |
