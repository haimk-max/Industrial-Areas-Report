<!-- RENDERED ARTIFACT — DO NOT EDIT BY HAND.
     Generated by scripts/render_zone_prompt.py --step report at 2026-06-14T15:57:06Z
     template=zone_report_prompt_template_v5.md
     template_sha256_12=d28d4cc525fb
     diagnosis_sha256_12=9c557d628af5
  total_active=7  measurements=499  years=2011-2025
...
<zone_metadata>
Zone name (Hebrew): אזה״ת רעננה
Zone name (English): Raanana
General monitoring boreholes: 3
Fuel boreholes: 2
Total active boreholes: 7
Measurements (TPFAS/BETK excluded): 499
Year range: 2011–2025
Focus order (from zone_diagnosis.md ## סדר מוקדים): 1. ליבת קריית אתגרים — מוביל: CVOC (max_bucket=8); מיקום: מרכז-מזרח קריית אתגרים
2. תחנת טורבינות גז — מוביל: PFAS (max_bucket=8); מיקום: צפון-מזרח האזור (ITM 189345/678575)
3. תחנת פז הנופר — מוביל: FUEL (max_bucket=3); מיקום: הנופר 36
4. פערי כיסוי — מוביל: PFAS (ואחרים) (max_bucket=—); מיקום: —
Precedent zone (style reference): Holon
...
1. ליבת קריית אתגרים — מוביל: CVOC (max_bucket=8); מיקום: מרכז-מזרח קריית אתגרים
2. תחנת טורבינות גז — מוביל: PFAS (max_bucket=8); מיקום: צפון-מזרח האזור (ITM 189345/678575)
3. תחנת פז הנופר — מוביל: FUEL (max_bucket=3); מיקום: הנופר 36
4. פערי כיסוי — מוביל: PFAS (ואחרים) (max_bucket=—); מיקום: —
...
<document index="1">
<source>Holon/output/HOLON_REPORT_V8.md (Approved Precedent)</source>
<purpose>Style reference ONLY: tone, section structure, citation format. **DO NOT copy structure** — אזה״ת רעננה's contamination story is different.</purpose>
</document>

<document index="2">
<source>Raanana/context_pack/03_context/reports_context.md (Reports Context Pack)</source>
...
<document index="3">
<source>Raanana/context_pack/03_context/source_candidates_context.md (Source Candidates Pack)</source>
...
<document index="4">
<source>Raanana/context_pack/04_diagnosis/zone_diagnosis.md (Zone Diagnosis — Opus Call #1)</source>
...
<document index="5">
<source>Raanana/lean_workspace/04_deterministic_anchors/statistical_signals_PILOT.yaml + forensic_anchors_PILOT.yaml (Structured Anchors PILOT)</source>
...
All CSVs are in `Raanana/02_data/`:

<document index="6a">
<source>zone_wells.csv</source>
<purpose>All 7 boreholes: IDs, coordinates, type, monitoring status.</purpose>
...
- Monitoring network (7 boreholes: composition, status)
- Figure 1 required (map with wells + contamination foci)

### Section 3: מוקדי זיהום עיקריים (Contamination Foci / Families)
- **Order**: by geographic focus per §IV (1. ליבת קריית אתגרים — מוביל: CVOC (max_bucket=8); מיקום: מרכז-מזרח קריית אתגרים
2. תחנת טורבינות גז — מוביל: PFAS (max_bucket=8); מיקום: צפון-מזרח האזור (ITM 189345/678575)
3. תחנת פז הנופר — מוביל: FUEL (max_bucket=3); מיקום: הנופר 36
4. פערי כיסוי — מוביל: PFAS (ואחרים) (max_bucket=—); מיקום: —); families secondary within each focus; "פערי כיסוי" last
...
- חלון: 5 שנים (2021–2025)
- מנין קידוחים: 3 ניטור כללי + 2 דלק = 7 (זה המספר היחיד שמותר לצמוד למילה "קידוחים")
...
- **Consistency (STRICT — QA gate)**: the ONLY multi-digit number that may directly precede the word "קידוח/קידוחים" is **7**. For any other count, keep a 2–3 digit number away from "קידוח": write "X מתוך 7 הקידוחים", "כ-N נקודות ניטור", or "N חריגות". A distinct "NN קידוחים" elsewhere FAILs the gate.
...
Before submitting Raanana_REPORT_V5.md, verify (PROCESS_GUIDE §VII):

- [ ] 6 sections + Methodology + Limitations + Appendices (V5 schema)
- [ ] All numbers tied to source (CSV row or historical document page)
- [ ] Focus order correct: §3 = geographic foci (חומרה יורדת); families secondary within focus; "פערי כיסוי" last (ראה §IV; order per 1. ליבת קריית אתגרים — מוביל: CVOC (max_bucket=8); מיקום: מרכז-מזרח קריית אתגרים
2. תחנת טורבינות גז — מוביל: PFAS (max_bucket=8); מיקום: צפון-מזרח האזור (ITM 189345/678575)
3. תחנת פז הנופר — מוביל: FUEL (max_bucket=3); מיקום: הנופר 36
4. פערי כיסוי — מוביל: PFAS (ואחרים) (max_bucket=—); מיקום: —)
...
- [ ] Methodology includes formula + borehole count (7) + MK description
