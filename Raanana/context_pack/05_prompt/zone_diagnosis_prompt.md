<!-- RENDERED ARTIFACT — DO NOT EDIT BY HAND.
     Generated by scripts/render_zone_prompt.py --step diagnosis at 2026-06-14T15:54:33Z
     template=zone_diagnosis_prompt_template.md
     template_sha256_12=068bb854d3db
...
You are a senior hydrogeochemist and environmental forensics specialist preparing a professional Zone Diagnosis for `אזה״ת רעננה` (id: `raanana`).
...
> Paths below use the canonical V5 data-pack naming (`DATA_PIPELINE_SPEC.md`) and the deterministic-anchors location. The renderer (`scripts/render_zone_prompt.py --step diagnosis`) fills `Raanana/02_data`, `Raanana/lean_workspace/04_deterministic_anchors`, `Raanana/forensics`, `Raanana/context_pack/03_context` to the real zone paths.

| # | File | Purpose |
|---|------|---------|
| 1 | `Raanana/lean_workspace/04_deterministic_anchors/statistical_signals_PILOT.yaml` | Statistical anchors (S1–S11 / S99) — navigation aid |
| 2 | `Raanana/lean_workspace/04_deterministic_anchors/forensic_anchors_PILOT.yaml` | Forensic anchors (F1–F12 / F99) — navigation aid |
| 3 | `Raanana/02_data/trends_by_well_parameter.csv` | Mann-Kendall results per (well, parameter) |
| 4 | `Raanana/02_data/measurements_scoped.csv` | Scoped measurements (window-filtered) |
| 5 | `Raanana/lean_workspace/04_deterministic_anchors/severity_index_2025_raanana.csv` | Severity per (well, family) |
| 6 | `Raanana/lean_workspace/04_deterministic_anchors/severity_index_2025_raanana_param_level.csv` | Severity per (well, family, parameter) |
| 7 | `Raanana/forensics/contamination_families.json` | Decay chains, source signatures, co-occurrence, critical exceedances |
| 8 | `Raanana/context_pack/03_context/reports_context.md` | Prior zone reports / baseline interpretations |
| 9 | `Raanana/context_pack/03_context/source_candidates_context.md` | Regional facilities / source candidates context |

### Optional (use if present in the zone)

| File | Purpose |
|---|---|
| `Raanana/context_pack/03_context/web_findings_context.md` | Current activity / status from web sources (use with classification **C** — status only, not contamination proof) |
| `Raanana/context_pack/03_context/source_candidates_index.csv` | Tabular source candidates with evidence classification A–E |
| `Raanana/lean_workspace/04_deterministic_anchors/monitoring_gaps.csv` | Pre-computed monitoring gaps if available |
...
- **Path**: `Raanana/context_pack/04_diagnosis/zone_diagnosis.md` (typically `Raanana/context_pack/03_context/04_diagnosis/zone_diagnosis.md`)
...
2. **Then read the severity indices** (`severity_index_2025_raanana.csv` and `_param_level.csv`) — these summarize the contamination picture per well per family.
...
Write the Zone Diagnosis for `אזה״ת רעננה` to `Raanana/context_pack/04_diagnosis/zone_diagnosis.md`. Start directly with `# אבחון אזורי — אזה״ת רעננה` (no preamble, no commentary, no markdown fence).
