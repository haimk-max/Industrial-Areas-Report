...
![מטריצת חומרה — אינדקסי משפחות לפי קידוח](../charts_v2/designed/fig_02_severity_matrix.html)
...
הסעיף מסודר לפי הסדר הקבוע **<bdi>CVOC → METALS</bdi> → PFAS → FUEL** — לפי משמעות סביבתית-אזורית, ולא לפי ספירת קידוחים או ריכוזים מוחלטים.
...
מבין מגמות העלייה במתכות, הבולטת היא **נת חולון 14** עם Cr עולה (Z=1.87, p=0.062, ריכוז עדכני 166 µg/L = 331% מהתקן) — קידוח בצפון האזור ליד מפעלי הציפוי ההיסטוריים (אמקור פליז, המלבין, סופר כרום); ציר הריכוזים נת חולון 14  — נת חולון 26 עקבי עם כיוון הזרימה SW. ב**מק חולון 14** (קידוח הפקה) זוהתה מגמת עלייה ב-As (Z=1.66, p=0.096) בערכים מתחת לתקן.

**Co-occurrence — חתימת ציפוי מתכת (Cr+Ni)**: שמונה קידוחים מציגים נוכחות מקבילה של Cr ו-Ni מעל 10% מהתקן. הבולטים: **נת תדירגן 4** (Cr 133 µg/L = 266%, Ni 2.6 µg/L, As 1.15 µg/L, 2011), **נת חולון 23** (Ni 39 µg/L = 196%, Cr 45 µg/L = 90%, 2022) ו**נת חולון 22** (Cr 45 µg/L = 90%, Ni 13 µg/L = 63%, 2023); נוספים: נת חולון 25, נת אלביט חולון 1–2, ו**נד פז סיירים 4** עם Ni 240 µg/L (1,202%, 2021 — אינדקס 4) שמהווה חתימה לא טיפוסית לתחנת דלק. חתימת Cr+Ni אופיינית לפסולת ציפוי גלוואני (כרומיום שש-ערכי + ניקל מציפוי כפול); 16 מפעלי הציפוי שזיהה אקולוג ב-2012 (חלק 1) הם מקור החשד הראשי.
...
1. **PFAS — פער כיסוי דרמטי**: מק חולון 12 נדגם לPFAS בלבד פעם אחת (ND). בעומק של מק חולון 12 (הפקה), יתכנו מקורות PFAS מעלה (תחנות דלק עתיקות עם AFFF, מפעלי ציפוי עם PFOS) שלא תופסו בדגימה היחידה. **דרישה דחופה: דגימה חוזרת של מק חולון 12 ו-23 עם פנל PFAS מורחב ב-8–10 מדידות.**

2. **As עולה במק חולון 14**: אמנם מתחת לתקן השתייה (10 µg/L), אך המגמה היא מובהקת סטטיסטית (Z=1.66, p=0.096). As הוא מזהם קרצינוגני שעלייתו במק חולון 14 מצביעה על שינוי בתנאים הידרוגיאולוגיים או חדרת זיהום ממוקד קרוב. **דרישה: מעקב הדוק עם דגימות רבעוניות ב-2026 לאישור/הכחשת המגמה.**

3. **ממצעות משקללות לאזור כולו**: קידוחי הפקה רחוקים מהמוקדים התעשייתיים הישירים (2–3 ק"מ דרומה/מעלה). ממצאים בהם מהווים אינדיקטור לתנועת פלומות ארוכות טווח והשאיבה של מי תהום מזוהמות לכיוון מתקני היצור. **מק חולון 14 (בצפון-מזרח) במיוחד קרוב למפעלי הציפוי ההיסטוריים; As עולה בו עקבי עם הקרבה למקורות Cr+Ni משוער.**

4. **רמת ביטחון פנימית**: קידוחי הפקה מנוהלים ע"י מתקני מי קטנים או מזניקי מים מקומיים (לא תמיד חברת המים הלאומית). **פיקוח רוטיני על איכות מי הפקה בקידוחים אלה חיוני לתפעול בטוח של מערכות המזניקים.**
...
*גרסה 4.1 (מאי 2026) — סדר משפחות <bdi>CVOC → METALS</bdi> → PFAS → FUEL; טרמינולוגיה עברית בלבד; מתודולוגיה מורחבת (נוסחת אינדקס, מנין קידוחים, חלון זמן, קריטריון תפעולי); סעיף 4b — חמישה מוקדי זיהום גיאוגרפיים; פער PFAS מסומן כממצא מתודולוגי.*
