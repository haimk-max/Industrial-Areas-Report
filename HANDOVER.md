...
- **`monitoring_gaps.csv` תלוי-זמן (`datetime.now()`) — אי-דטרמיניות** (זוהה 2026-06-18, REQ #36): `generate_zone_data_pack.py:324` קובע `is_active` לפי `(datetime.now() - last_measurement).days < 730`. המשמעות: **הרצה-מחדש של הפייפליין בימים שונים מפיקה monitoring_gaps.csv שונה** גם בלי שום שינוי בנתונים — כל קידוח שחוצה את גבול ה-730 יום (שנתיים מהדגימה האחרונה) מתהפך מ-`active` ל-`closed`. דוגמה: ב-2026-06-18 נד_פז_סיירים_4 (דגימה אחרונה 2024-06-17) התהפך, בעוד שב-commit מ-2026-06-16 היה עדיין active. **6 הקבצים האחרים של ה-Data Pack דטרמיניסטיים** (boreholes/zone_wells/measurements_scoped משוחזרים byte-for-byte). **כלל ביניים**: אל תתייחס ל-diff ב-monitoring_gaps.csv לבדו כ"שינוי נתונים" — בדוק אם זה רק שדה is_active/reason. **תיקון-שורש מתוכנן** (REQ #36): להחליף `datetime.now()` בעוגן-תאריך נגזר-נתונים (anchor date).
